...
# [mm] width, length indicating ellipse centre
position_ellipse = (width/2., length/2.)
# [mm] half of the hole size axis along [x, y] axis.
dimension_ellipse = (diameter/2., diameter/2.)
...
# The elliptic hole edge has 4x seedsOnEdge to generate radial mesh
seedsOnHole = 4*seedsOnEdge
# ref nodes' starting index, must be new nodes, hence a large starting index.
maxNodeID = 10*seedsOnHole**2
...
def model_generator(modelName, epsilon_1, epsilon_2, epsilon_12, runJob=False, ncpus=1):
    ...
    # create the sketch
    currentSketch = currentModel.ConstrainedSketch(
        name='2D_sketch', sheetSize=float(np.max((width, length, depth))))
    # draw rectangle
    ...
    currentPart = currentModel.Part(
        name='part_3D', dimensionality=THREE_D, type=DEFORMABLE_BODY)
    ...
    sectionRegion = (currentPart.faces.findAt(
        (width*0.99, length*0.99, 0.),),)  # location on the plate
    ...
    currentAssembly.Instance(name='Part Instance',
                             part=currentPart, dependent=ON)
    ...
    edges_ellipse = edgefind(
        ((width/2.-dimension_ellipse[0], length/2., 0.),),)
    ...
    currentPart.setMeshControls(
        algorithm=MEDIAL_AXIS, elemShape=QUAD, regions=currentMeshRegion)
    currentPart.setElementType(
        regions=currentMeshRegion, elemTypes=(meshElemType,))

    # define mesh seeds
    currentPart.seedPart(deviationFactor=0.1,
                         minSizeFactor=0.1, size=seedsOnEdge)
    currentPart.seedEdgeByNumber(
        constraint=FINER, edges=edges_ellipse, number=seedsOnHole)
    for edge in (edges_l, edges_r):
        currentPart.seedEdgeByNumber(
            constraint=FINER, edges=edge, number=seedsOnEdge)
    for edge in (edges_b, edges_t):
        currentPart.seedEdgeByNumber(
            constraint=FIXED, edges=edge, number=seedsOnEdge)
    ...
    currentModel.fieldOutputRequests.changeKey(
        fromName='F-Output-1', toName='FieldOutputGlobal')
    currentModel.fieldOutputRequests['FieldOutputGlobal'].setValues(
    ...
    currentModel.historyOutputRequests.changeKey(
        fromName='H-Output-1', toName='HistoryOutputs')
    currentModel.historyOutputRequests['HistoryOutputs'].setValues(
        variables=PRESELECT)
    ...
    # matching nodes for PBC in DoF 1 and DoF 2 (global coordinate system)
    pairs1, pairs2 = [], []
    edgelabels = []

    for set_name in ['edge_left', 'edge_right']:     # loop over direction 1
        # loop over nodes in set
        for i_node in range(len(currentAssembly.sets[set_name].nodes)):
            # save node number and coordinates in list
            label = currentAssembly.sets[set_name].nodes[i_node].label
            coords = currentAssembly.sets[set_name].nodes[i_node].coordinates
            # list of lists, each sublist contains node label and coordiates
            pairs1.append([label]+list(coords))

            # define set (required for equation constraint)
            currentAssembly.SetFromNodeLabels(
                name='Node_' + str(label), nodeLabels=(('Part Instance', (label,)),))
    ...
    # data type specification needed for sorting
    nodelst_dtype = [('label', 'i4'), ('x', 'f4'), ('y', 'f4'), ('z', 'f4')]
    pairs1, pairs2 = np.array([tuple(i) for i in pairs1], dtype=nodelst_dtype), np.array(
        [tuple(i) for i in pairs2], dtype=nodelst_dtype)

    # sort pair-lists based on coordinates such that the first half of the list is edge A and the second half is edge B
    # first x to divide the sides, then y to sort sequence on edge
    pairs1.sort(axis=0, order=['x', 'y'])
    # first y to divide the sides, then x to sort sequence on edge
    pairs2.sort(axis=0, order=['y', 'x'])
    # indices of mid-points, which is the division between edges
    mp1, mp2 = len(pairs1)/2, len(pairs2)/2

    # define reference points to be used for loading and define a set for each
    # define them to be larger than the max node ID in the mesh to avoid conflict
    refNnormal = maxNodeID+1
    refNshear = maxNodeID+2
    RP_normal_node = currentPart.Node(coordinates=(
        width*1.1, length/2., 0.)).setValues(label=refNnormal)
    RP_shear_node = currentPart.Node(coordinates=(
        width*-0.1, length/2., 0.)).setValues(label=refNshear)
    currentAssembly.SetFromNodeLabels(
        name='RP_normal', nodeLabels=(('Part Instance', (refNnormal,)),))
    currentAssembly.SetFromNodeLabels(
        name='RP_shear', nodeLabels=(('Part Instance', (refNshear,)),))

    # equation constraints
    # constraints on pairs in x-direction
    for i_pair in range(0, int(np.shape(pairs1)[0])/2):
        # combination of node numbers
        equationName = 'eq_' + \
            str(pairs1[i_pair][0])+'_'+str(pairs1[i_pair+mp1][0])
    ...
    # constraints on pairs in y-direction
    for i_pair in range(0, int(np.shape(pairs2)[0])/2):
        # combination of node numbers
        equationName = 'eq_' + \
            str(pairs2[i_pair][0])+'_'+str(pairs2[i_pair+mp2][0])
        ...
        # and i_pair != int(np.shape(pairs2)[0]-2): # exclude left pair to avoid overconstraint
        if i_pair != 0:
    ...
    currentFaces = currentInstance.faces.findAt(
        ((width*0.99, length*0.99, 0.),),)
...
def extractor_StrainStress(modelName):
    ...
    # session is imported from abaqus
    ...
    for frame in step.frames:
        ...
        if abs(gamma_12) > 0.:  # avoid division by 0
            sig_12 = (RF_12[0]*U_12[0]+RF_12[1]*U_12[1]) / \
                width/length/depth/gamma_12
        else:
            sig_12 = 0.
...
def extractor_DamagePatterns(modelName, damageLabels, Plies, Nfiles=10):
    ...
    k_list = np.linspace(StartFrame, EndFrame, Nfiles).astype(
        int)  # form list of frames to print
    ...
    for k in k_list:
        ...
        for label in damageLabels:
            ...
            for i in Plies:
                ...
                # Select layer i
                myViewport.odbDisplay.setPrimarySectionPoint(
                    activePly="PLY-" + str(i))
                ...
                # Print layer i
                fileName = label+'_Ply' + \
                    str(i) + '_' + modelName + '_T' + str(int(currentTime*1e6))
                session.printToFile(fileName=fileName,
                                    format=PNG, canvasObjects=(myViewport, ))
